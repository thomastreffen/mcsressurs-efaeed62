...
const PERMISSION_GROUPS: Record<string, { label: string; keys: string[] }> = {
  scope: {
    label: "Scope / Omfang",
    keys: ["scope.view.own", "scope.view.company", "scope.view.all"],
  },
  jobs: {
    label: "Jobber",
  ...
  offers: {
    label: "Tilbud & Kalkyle",
    keys: ["offers.view", "offers.create", "offers.edit", "offers.delete", "offers.archive", "calc.view", "calc.edit"],
  },
  docs: {
    label: "Dokumenter",
          ...
          <Card key={r.id} className="cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => openEdit(r)}>
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">{permCount} tillatelser</p>
              </div>
