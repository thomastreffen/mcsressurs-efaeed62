...
export default function KpiDashboard() {
  ...
  useEffect(() => {
    async function fetchKpi() {
      ...
      const [eventsRes, techsRes] = await Promise.all([
        supabase
          .from("events")
          .select("id, title, status, customer, internal_number, start_time, end_time, event_technicians(technician_id, technicians(name, color))")
      ...
      const scheduledThisWeek = thisWeekEvents.filter((e: any) =>
        ["scheduled", "approved", "in_progress"].includes(e.status)
      ).length;
      ...
      const techHours = Array.from(techHoursMap.values())
        .filter((t) => t.hours > 0)
      ...
      setData({ totalActive, scheduledThisWeek, completedThisWeek, pendingChanges, rejected, techHours, recentJobs });
      setLoading(false);
    }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        />
        <KpiCard
          title="Planlagt denne uke"
          value={data.scheduledThisWeek}
          icon={<Clock className="h-4 w-4" />}
          accent="text-status-scheduled"
        />
        <KpiCard
          title="Fullførte denne uke"
        ...
        <KpiCard
          ...
          accent="text-status-time-change-proposed"
          highlight={data.pendingChanges > 0}
        />
        <KpiCard
          title="Avslått"
          value={data.rejected}
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-24 h-2 rounded-full bg-secondary overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min((tech.hours / 40) * 100, 100)}%`,
